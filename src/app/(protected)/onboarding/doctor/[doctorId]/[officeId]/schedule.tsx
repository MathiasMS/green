...
// Días de la semana
const Days: { id: DayId; name: DayName }[] = [
  { id: 'L', name: 'Lunes' },
  { id: 'M', name: 'Martes' },
  { id: 'X', name: 'Miércoles' },
  { id: 'J', name: 'Jueves' },
...
type DayId = 'L' | 'M' | 'X' | 'J' | 'V' | 'S' | 'D';
type DaysSelected = Record<DayId, boolean>;
type DayName = 'Lunes' | 'Martes' | 'Miércoles' | 'Jueves' | 'Viernes' | 'Sábado' | 'Domingo';
type ScheduleRange = Record<DayName, { startTime: string; endTime: string }[]>;
...
const OfficeScheduleScreen = () => {
  ...
  const actualizarHorario = (day: DayName, index: number, type: string, value: string) => {
    const newSchedules = { ...schedules };
  ...
  const onSubmit = async () => {
    try {
      ...
      router.push({
        pathname: '/(protected)/onboarding/doctor/[doctorId]/specialization',
        params: { doctorId },
      });
    } catch (error) {
            ...
            {Days.map(day => {
              ...
              return (
                <View key={day.name} className="mb-8">
                  <Text className="text-xl font-bold mb-3">{day.name}</Text>
                    ...
                    <View key={index} className="flex-row items-center mb-4">
                      ...
                      <View className="flex-1">
                        <TimeSelector
                          ...
                          onSelect={value => actualizarHorario(day.name, index, 'startTime', value)}
                        />
                      </View>

                      <Text className="mx-2 text-base">a</Text>

                      {/* Selector de hora de fin */}
                      <View className="flex-1">
                        <TimeSelector
                          value={range.endTime}
                          onSelect={value => actualizarHorario(day.name, index, 'endTime', value)}
                        />
                      </View>

                      {/* Botones para agregar/eliminar rangos */}
                      <View className="flex-row ml-2 w-10">
                        {index > 0 && (
                          <Pressable
                            onPress={() => {
                              const newSchedules = { ...schedules };
                        ...
                        {index === schedules[day.name].length - 1 && (
                          <Pressable
